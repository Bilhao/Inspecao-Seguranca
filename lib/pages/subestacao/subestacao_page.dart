...
class SubestacaoPage extends StatefulWidget {
  const SubestacaoPage({super.key});

  @override
  State<SubestacaoPage> createState() => _SubestacaoPageState();
}
...
class _SubestacaoPageState extends State<SubestacaoPage> {
  ...
  Widget build(BuildContext context) {
    return Consumer<SubestacaoProvider>(
      builder: (context, provider, child) {
        return Scaffold(
          appBar: AppBar(
            title: const Text('Inspeção de Subestação', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            centerTitle: true,
          ...
          body: Padding(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  const Text("Escolha a Subestação: ", style: TextStyle(fontSize: 16)),
                  const Divider(height: 15, color: Colors.transparent),
                  DropdownMenu(
                    width: double.maxFinite,
                    initialSelection: provider.subestacao,
                    onSelected: (value) {
                      setState(() {
                        dropMenuerrorText = null;
                      });
                      provider.subestacao = value.toString();
                    },
                    ...
                    dropdownMenuEntries: subestacoes
                        .map((subestacao) => DropdownMenuEntry(value: subestacao, label: subestacao))
                        .toList(),
                  ),
          ...
          floatingActionButton: MediaQuery.of(context).viewInsets.bottom != 0.0
            ...
            : FloatingActionButton.extended(
              onPressed: () {
                if (provider.subestacao == "" || provider.inspetor1 == "") {
                  ...
                  Navigator.pushNamed(context, "/inspecao-subestacao", arguments: {"id": 0});
                }
              },
              backgroundColor: Theme.of(context).colorScheme.primary,
