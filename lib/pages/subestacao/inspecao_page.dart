...
class InspecaoSubestacaoPage extends StatefulWidget {
  const InspecaoSubestacaoPage({super.key});

  @override
  State<InspecaoSubestacaoPage> createState() => _InspecaoSubestacaoPageState();
}

class _InspecaoSubestacaoPageState extends State<InspecaoSubestacaoPage> {
  @override
  Widget build(BuildContext context) {
    return Consumer(
      builder: (context, SubestacaoProvider provider, child) {
        return Scaffold(
          appBar: AppBar(
            title: const Text('Locais da Subestação', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            centerTitle: true,
                          ...
                          child: Column(
                            ...
                            children: [
                              Text.rich(
                                TextSpan(
                                  children: [
                                    TextSpan(text: provider.subestacao, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
                                  ],
                                ),
                              ),
                              Text.rich(
                            ...
                            for (int i = 0; i < provider.itensId.length; i++)
                              Container(
                                ...
                                child: FilledButton.tonal(
                                  onPressed: () => Navigator.pushNamed(context, "/perguntas-subestacao", arguments: {"id": i}),
                                  style: ButtonStyle(
          ...
          floatingActionButton: MediaQuery.of(context).viewInsets.bottom != 0.0
            ...
            : FloatingActionButton.extended(
              onPressed: () {
                Navigator.pushNamed(context, "/inspecao-subestacao-resumo");
              },
            backgroundColor: Theme.of(context).colorScheme.primary,
              heroTag: "unique_tag_subestacao",
