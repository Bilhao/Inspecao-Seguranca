...
class ResumoInspecaoPredioPage extends StatelessWidget {
  const ResumoInspecaoPredioPage({super.key});

  @override
  Widget build(BuildContext context) {
    final provider = Provider.of<PredioProvider>(context, listen: false);
    ...
    return Scaffold(
          ...
          child: Column(
            children: [
                          ...
                          Text.rich(
                            TextSpan(
                              children: [
                                ...
                                TextSpan(text: provider.predio, style: const TextStyle(fontSize: 15)),
                              ],
                            ),
                          ),
                          Text.rich(
              ...
              Table(
                ...
                columnWidths: const <int, TableColumnWidth>{0: IntrinsicColumnWidth(flex: 1), 1: IntrinsicColumnWidth(flex: 2), 2: IntrinsicColumnWidth(flex: 5)},
                children: [
                  const TableRow(
                    children: [
                    ...
                    for (var pergunta in provider.perguntas[itemId]!.entries) ...[
                      TableRow(
                        ...
                        children: [
                          ...
                          Padding(
                            padding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 5.0),
                            child: Text(provider.respostas[itemId]![pergunta.key] == null ? "---" : "${provider.respostas[itemId]![pergunta.key]}", style: const TextStyle(fontSize: 15)),
                          ),
                          Padding(
                            padding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 5.0),
      ...
      floatingActionButton: MediaQuery.of(context).viewInsets.bottom != 0.0
          ...
          : FloatingActionButton.extended(
              onPressed: () {
                showDialog(context: context, builder: (context) => _buildSaveAlertDialog(context, provider));
              },
  ...
  Widget _buildSaveAlertDialog(BuildContext context, PredioProvider provider) {
    return AlertDialog(
      icon: Icon(Icons.save, color: Theme.of(context).colorScheme.primary),
      title: const Text('Salvar dados da inspeção localmente ou enviar por email?', style: TextStyle(fontSize: 20), textAlign: TextAlign.center),
      ...
      actions: [
        TextButton(
          ...
          onPressed: () async {
            if (await prediotoxlsx(provider: provider, sendEmail: false) && context.mounted) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Erro ao salvar dados!"), showCloseIcon: true));
            }
          },
        ),
        TextButton(
  ...
  Widget _buildEmailAlertDialog(BuildContext context, PredioProvider provider) {
    TextEditingController emailController = TextEditingController();
    return AlertDialog(
      icon: Icon(Icons.save, color: Theme.of(context).colorScheme.primary),
      title: const Text('Insira o email do destinatário:', style: TextStyle(fontSize: 20), textAlign: TextAlign.center),
      content: SizedBox(
        width: double.maxFinite,
        child: TextField(
      ...
      actions: [
        TextButton(
          ...
          onPressed: () async {
            ...
            bool saved = await prediotoxlsx(provider: provider, sendEmail: true, destinatario: emailController.value.text);
            if (!context.mounted) return;
            if (saved) {
              Navigator.of(context).pushNamedAndRemoveUntil("/", (Route<dynamic> route) => false);
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Email enviado com sucesso!")));
              provider.eraseSavedInfos();
            } else {
              Navigator.of(context).pop();
              Navigator.of(context).pop();
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Erro ao enviar email!"), showCloseIcon: true));
            }
          },
        ),
      ],
